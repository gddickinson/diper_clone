"""
Implementation of the Direction Autocorrelation analysis from DiPer.
"""
import os
import numpy as np
import pandas as pd
...
def normalize_vectors(traj: pd.DataFrame) -> pd.DataFrame:
    """
    Calculate normalized vectors for a trajectory.
...
def calculate_autocorrelation(traj: pd.DataFrame, 
    ...
    for step in range(1, max_intervals + 1):
        ...
        for i in range(len(traj) - step):
            ...
            if (traj['x_vec'].iloc[i] == 0 and traj['y_vec'].iloc[i] == 0) or \
               (traj['x_vec'].iloc[i + step] == 0 and traj['y_vec'].iloc[i + step] == 0):
                continue
...
def plot_autocorrelation(autocorr_results: Dict[str, pd.DataFrame],
    ...
    # Customize plot
    ax.set_title("Direction Autocorrelation", fontsize=14)
...
def run_autocorrelation_analysis(data: Dict[str, pd.DataFrame], 
                               time_interval: float,
                               max_intervals: int = 30,
                               output_dir: str = "output") -> Dict[str, pd.DataFrame]:
    """
    Run the Direction Autocorrelation analysis for all conditions.
    ...
    for condition_name, df in data.items():
        ...
        vector_trajectories = [normalize_vectors(traj) for traj in trajectories]
        ...
        # Save results
        save_results(avg_autocorr, output_dir, f"autocorr_{condition_name}")
        ...
        # Save statistics
        save_results(stats_df, output_dir, f"autocorr_stats_{condition_name}")
    
    # Plot autocorrelation
    if all_autocorr_results:
        fig = plot_autocorrelation(all_autocorr_results, output_dir)
        save_figure(fig, output_dir, "autocorrelation")
        plt.close(fig)
...
if __name__ == "__main__":
    ...
    # Run analysis
    run_autocorrelation_analysis(test_data, time_interval=1.0, max_intervals=30, output_dir="test_output")
